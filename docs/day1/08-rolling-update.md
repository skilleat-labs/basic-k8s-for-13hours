...
- [Lab 7](07-scaling.md) 까지 마쳐 Deployment `web` 이 실행 중(replicas 4개 예상).
...
!!! note "수고하셨습니다"
    오늘 Pod 를 만들고, IP 가 휘발성임을 겪고, Deployment 로 자가복구·스케일링·무중단 배포까지 손으로 해 봤습니다. 그런데 [Lab 4](04-pod-ip-ephemeral.md) 에서 남은 숙제가 있죠 — "IP 가 계속 바뀌는데 어떻게 안정적으로 접속하지?" **2일차에서 이어집니다.** Service 로 그 문제를 해결하고, 클러스터 안팎의 네트워킹을 다룹니다.
