# Lab 1 · 볼륨 실습 — emptyDir

컨테이너 파일시스템은 임시적입니다. 가장 단순한 볼륨 **emptyDir** 로 컨테이너 간 데이터를 공유해 보고, Pod를 삭제하면 그 데이터가 사라지는 것을 직접 확인합니다. 이것이 다음 실습의 PV/PVC가 필요한 이유입니다.
...
> **결론**: emptyDir은 임시 데이터나 컨테이너 간 파일 공유에 적합하지만, Pod 삭제 후에도 데이터를 유지하려면 **PersistentVolume(PV) / PVC**가 필요합니다. → 다음 실습(PV / PVC)에서 계속합니다.

---

다음: [Lab 2 · 볼륨 실습 — PV / PVC](02-pv-pvc.md)
